...
\*/
	...
	function StringView(
		...
		if (nTranscrType < 8) {
			...
			if (nTranscrType & 4) {
				...
				nEndIdx = nCharEnd =
					(Number.isInteger(nLength) ? Math.min(Math.max(nLength, 0) + nStartIdx, nOutptLen) : nOutptLen) - 1
			} else {
				/* input is stringView */
			...
			if (
				nOutptLen === 0 ||
				(nTranscrType < 4 && vSource.encoding === this.encoding && nCharStart === 0 && nOutptLen === nInptLen)
			) {
	...
	StringView.putUTF8CharCode = function (aTarget, nChar, nPutAt) {
		...
		if (nChar < 0x80 /* 128 */) {
			...
			/* six bytes */
			aTarget[nIdx++] =
				0xfc /* 252 */ + /* (nChar >>> 32) is not possible in ECMAScript! So...: */ nChar / 1073741824
	...
	StringView.makeFromBase64 = function (sB64Inpt, sEncoding, nByteOffset, nLength) {
		return new StringView(
			sEncoding === "UTF-16" || sEncoding === "UTF-32"
				? StringView.base64ToBytes(sB64Inpt, sEncoding === "UTF-16" ? 2 : 4).buffer
	...
	StringView.prototype.subview = function (nCharOffset /* optional */, nCharLength /* optional */) {
		...
		nStrLen = Number.isInteger(nCharLength)
			? Math.max(nCharLength, 0) + nCharStart > nStringLength
				? nStringLength - nCharStart
				: nCharLength
			: nStringLength
